...
// Purpose: Hold TXQ ids, mempool resources
//
// Classes:
...
// +-----------------------+---------------+------------+-------------------------------------------------------------+
// | id                    | int           | 0          | Name or number of this TXQ                                  |
// +-----------------------+---------------+------------+-------------------------------------------------------------+
// | vcpuId                | int           | 0          | Name or number of VCPU running this TXQ i.e. the VCPU to    |
// |                       |               |            | which the thread running this queue is pinned.              |
// +-----------------------+---------------+------------+-------------------------------------------------------------+
// | lcoreId               | int           | 0          | Name or number of the lcore running this TXQ                |
// +-----------------------+---------------+------------+-------------------------------------------------------------+
// | memzone               | rte_memzone * | 0          | the memzone from which this TXQ's mempool was allocated from|
// +-----------------------+---------------+------------+-------------------------------------------------------------+
// | mempool               | rte_mempool * | 0          | the mempool to be used for this TXQ                         |
// +-----------------------+---------------+------------+-------------------------------------------------------------+
...
public:
  ...
  TXQ(int id, int vcpuId, int lcoreId);
    // Create a object with specified parameters. The behavior is defined provided integer arguments are greater than
    // or equal to zero. Attributes not appearing in function list are defaulted.

  TXQ();
    // Create TXQ object with default values

  TXQ(const TXQ& object);
...
// FREE OPERATORS
std::ostream& operator<<(std::ostream& stream, const TXQ& object);
...
inline                                                                                                                  
TXQ::TXQ(int id, int vcpuId, int lcoreId)
: d_id(id)                                                                                                              
, d_vcpuId(vcpuId)                                                                                                      
, d_lcoreId(lcoreId)                                                                                                    
, d_memzone(0)
...
inline
int TXQ::vcpuId() const {
  return d_vcpuId;
}

inline
int TXQ::lcoreId() const {
  return d_lcoreId;
}

inline
const rte_memzone *TXQ::memzone() const {
  return d_memzone;
}

inline
rte_mempool *TXQ::mempool() const {
  return d_mempool;
}

inline
int TXQ::ringSize() const {
  return d_ringSize;
}

inline
const std::string& TXQ::memzonePolicy() const {
  return d_mempoolPolicy;
}

inline
int TXQ::defaultFlow() const {
...
inline
void TXQ::setMempool(rte_mempool *value) {
  assert(value);
  d_mempool = value;
}

inline
void TXQ::setRingSize(int value) {
  assert(value>0);
  d_ringSize = value;
}

inline
void TXQ::setMempoolPolicy(const std::string& value) {
